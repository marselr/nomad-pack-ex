...
```
nomad-pack run psmarsel --var message="Hola Mundo!"
```
...
balance the Consul service `psmarsel-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  psmarsel application
- `consul_service_name` (string) - The consul service name for the psmarsel
  application
